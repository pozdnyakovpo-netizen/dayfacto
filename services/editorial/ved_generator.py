...
def _check(d: VedDraft, facts: str, source: str = "") -> None:
    ...
    _t = " ".join([d.headline, d.what_changes, d.who, d.what_to_do]).lower()
    _f = facts.lower()
    for w in ("приложени", "перечн", "штраф", "ответственност", "санкци"):
        if w in _t and w not in _f:
            d.problems.append("утверждение вне данных: %s" % w)
...
def generate(router, change: dict, story_id: int = 0,
    ...
    user = (
        ...
        "Сохраняй охват требования дословно: если сказано \u00abвсе, кроме исключений, определяемых органом\u00bb, так и пиши. НЕ заменяй на перечень, приложение или список, если самого перечня в данных нет.\n"
        "Не добавляй последствия (штрафы, задержки, ответственность), если о них не сказано в данных.\n"
        "Если назван оператор, орган или номер документа, укажи их.\n\n"
        "ДЛИНА: заголовок до 90 знаков, what_changes до 320, "
        "who до 200, what_to_do до 220. Пиши плотно, без вводных "
        "оборотов и повторов. Общий объём поста не больше 850 знаков.\n\n"
        "Если стадия draft — это проект, пиши <проект>, а не <вступает в силу>.\n"
        "Если ограничение временное, укажи срок окончания.\n"
        "Напиши пост по данным ниже. Используй ТОЛЬКО эти факты. "
        "Верни JSON с ключами: headline, what_changes, who, what_to_do. "
        "Ровно эти ключи, без вложенности.\n\n"
        "ДАННЫЕ:\n" + facts
    )
